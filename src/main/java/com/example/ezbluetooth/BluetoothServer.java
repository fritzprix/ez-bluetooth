...
public class BluetoothServer {
    private static final String TAG = BluetoothServer.class.getCanonicalName();
    private static final int STATE_STARTING = 0x01;
    private static final int STATE_STARTED = 0x02;
    private static final int STATE_FINISHING = 0x03;
    private static final int STATE_FINISHED = 0x04;

    private BluetoothAdapter mBluetoothAdapter;
    private BluetoothManager mBluetoothManager;
    private ThreadPoolExecutor mPoolExecutor;
    private List<BluetoothServiceServer> mServices;
    ...
    public BluetoothServer(Context context, Callback callback) {
        wrCallback = new WeakReference<Callback>(new SyncCallback(callback));
        mBluetoothManager = (BluetoothManager) context.getSystemService(Context.BLUETOOTH_SERVICE);
        mBluetoothAdapter = mBluetoothManager.getAdapter();
    }

    public synchronized void start(BluetoothServiceServer...services) {
        ...
        for(final BluetoothServiceServer service : mServices) {
    ...
    public synchronized void stopService(int svcId) {
        BluetoothServiceServer serviceServer = mServices.remove(svcId);
